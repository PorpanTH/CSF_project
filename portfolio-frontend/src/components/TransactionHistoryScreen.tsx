...
export const TransactionHistoryScreen = ({ portfolioId }: TransactionHistoryScreenProps) => {
  ...
  const [typeFilter, setTypeFilter] = useState<'all' | 'sell'>('all')
  ...
  useEffect(() => {
    ...
    const loadHistory = async () => {
      try {
        setIsLoading(true)
        setError(null)
        const records = await portfolioAPI.getTransactionHistory(portfolioId, typeFilter === 'sell' ? { type: 'sell' } : undefined)
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="rounded-3xl border border-rose-200 bg-white shadow-sm overflow-hidden">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between px-6 py-5 border-b border-rose-100">
            <div>
              <h2 className="text-xl font-bold text-gray-900">Sell transactions</h2>
            ...
            <div className="relative inline-flex items-center">
              <select
                value={typeFilter}
                onChange={(e) => setTypeFilter(e.target.value as 'all' | 'sell')}
                ...
                <tbody className="divide-y divide-rose-50 bg-white">
                  {transactions.map(transaction => (
                    <tr key={transaction.id} className="hover:bg-rose-50/40 transition-colors">
                      <td className="px-6 py-4 text-gray-700 whitespace-nowrap">{transaction.saleDate}</td>
                      ...
                      <td className="px-6 py-4 text-right text-gray-700">${formatCurrency(transaction.salePrice)}</td>
