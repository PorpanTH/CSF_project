...
export const DepositModal = ({ onConfirm, onClose }: DepositModalProps) => {
  const [amount, setAmount] = useState<number>(0)

  const error = amount <= 0 ? 'Enter an amount greater than 0' : null
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-sm w-full mx-4">
        <div className="p-6">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Deposit funds to your portfolio
          </p>

          <div className="mt-4">
            <label className="block text-sm text-gray-600 mb-1">Amount to deposit</label>
            <input
              type="number"
              min={0}
              value={amount}
